...
def aggregate_features(df, target_threshold: float = None):
    ...
    ----------
    df : pd.DataFrame
        Must contain columns for both sources:
    ...
    # Ensure ordering
    df = df.sort_values(['interval_idx', 'time'])
    ...
    # Relative seconds from window start (shared time axis)
    df['rel_seconds'] = (
        df['time'] - df.groupby('interval_idx')['time'].transform('first')
    ).dt.total_seconds()
    ...
    def extract_one_source(group, prefix):
        ...
        # ----- Time regularity -----
        time_deltas = group['time'].diff().dt.total_seconds().dropna()
